<template>
    <div class="app-container">
      ...
      <el-card class="operate-container" shadow="never">
        ...
        <el-form-item label="管理员名称">
          <el-input v-model="searchObj.name" placeholder="管理员名称" />
        </el-form-item>
      ...
      <el-table
        ...
        <el-table-column prop="lastTime" label="创建时间" width="240" />
        <el-table-column label="操作" width="400" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="mini" @click="removeById(scope.row.id)">停用</el-button>
            <router-link :to="'/editReader/'+scope.row.id">
              <el-button type="text" size="mini">修改</el-button>
            </router-link>
            <el-button type="text" size="mini" @click="removeById(scope.row.id)">权限管理</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页组件 -->
  ...
  <script>
  import teacherApi from '@/api/reader'  // 这个引入方式是框架定义的，js文件的后缀js可省略。
  export default {
    // 定义数据模型
    data() { // 1、变量和初始值
      return {
